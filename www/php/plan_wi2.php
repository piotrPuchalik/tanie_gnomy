...
<html lang="pl-PL">
			...
			<table>
				<?php
					...
					if(isset($_GET['sala']))
					{
						$data=date("Y-m-d");
						$czas=date("H:i");
						$select = $conn->prepare("SELECT id, numerSali,typ_pomieszczenia FROM pomieszczenia WHERE budynek='WI2' AND numerSali=:numer");
						...
						if($result != null)
						{
							...
							{
								$select = $conn->prepare("SELECT p.id, p.imie, p.nazwisko, p.tytul FROM pracownicy as p JOIN pomieszczenia as s ON p.gabinet=s.id WHERE s.budynek='WI2' AND s.numerSali=:numer");
								$select->bindParam(':numer',$_GET['sala']);
								$select->execute();
								$result = $select->fetch();
	...
	<script src="../js/plan_wi2.js"> </script> 
	</body>
</html>